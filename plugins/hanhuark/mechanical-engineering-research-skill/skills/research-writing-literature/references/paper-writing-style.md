...
When data support it, include two to three quantitative findings in the abstract. These values should represent the main comparison, range, uncertainty, or validated performance measure, not incidental experimental detail.
...
Place a qualification near the finding it bounds. A useful qualification names the affected quantity, unmeasured boundary condition, untested regime, or model-form limitation and explains its consequence. Do not repeat generic disclaimers throughout the results section.
...
- Does the abstract report the most important quantitative findings when they are available?
- Do the introduction and literature review distinguish adjacent approaches from the specific remaining gap?
- Is each caveat specific, locally placed, and necessary for interpreting a claim?
