...
reference_dir="$skill_dir/references/standards"
skill_md="$skill_dir/SKILL.md"
common="$reference_dir/common-standards.md"
...
done < <(grep -oE 'references/standards/[A-Za-z0-9._-]+\.md' "$skill_md" | sort -u)

if [[ ! -f "$common" ]]; then
  echo "standards: canonical language-owner table is missing" >&2
  exit 1
fi
if [[ "$found_links" -eq 0 ]]; then
  echo "standards: domain SKILL.md has no link to the standards reference library" >&2
  fail=1
fi
...
done < <(find "$reference_dir" -maxdepth 1 -name '*.md' | sort)
