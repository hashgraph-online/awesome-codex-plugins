...
- When the delivery is marketing, editorial, campaign, or an explicitly new visual direction, add [`references/anti-slop.md`](references/anti-slop.md) as the visual-direction check, then [`references/impeccable.md`](references/impeccable.md) for a bounded direction and refinement pass. Both stay unselected for authenticated, internal, or convention-preserving product UI.
