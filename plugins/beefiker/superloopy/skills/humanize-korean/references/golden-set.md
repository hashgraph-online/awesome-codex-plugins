...
Repairs are shapes, not phrases: delete first, state the symptom from supplied facts second. See `docs/specs/2026-09-10-korean-calque-rules-design.md`.

### G-30 · P-1a · 리포트
audit: P-1a

```before
저장 요청이 서버에 도달하지 못하면 조용히 무시된다.
```

```after
저장 요청이 서버에 도달하지 못하면 무시된다.
```
...
```before
발표노트 태그가 사라지면 주간 이슈 등록이 조용히 실패해 0건이 된다.
```

```after
발표노트 태그가 사라지면 주간 이슈 등록이 에러를 내지 않고 실패해 0건이 된다.
```
...
```before
배포 트랙이 멈추면 재시작 전에 워커를 우아하게 종료한다.
```

```after
배포 트랙이 멈추면 재시작 전에 워커를 종료한다.
```
...
```before
주문 상태의 단일 진실 공급원은 orders 테이블이다.
```

```after
주문 상태의 기준 데이터는 orders 테이블이다.
```
...
```before
그는 실패를 인정하고 조용히 물러났다. 회의실은 조용한 분위기였다.
```
...
```before
이 문서에서 `단일 진실 공급원` 은 한 번만 정의한 용어다.
```
...
## Upstream v2.5–v2.7 Adoptions (2026-09-10)

### G-38 · A-21 · 칼럼
audit: A-21

```before
이 문제는 단순한 수지타산을 넘어 물가 안정과 고용의 균형을 묻는다.
```

```after
이 문제는 수지타산만이 아니라 물가 안정과 고용의 균형을 묻는다.
```
...
```before
지금 필요한 것은 속도가 아니라 방향이다. 팀은 분기마다 우선순위를 다시 정한다.
```

```after
지금은 속도보다 방향이 필요하다. 팀은 분기마다 우선순위를 다시 정한다.
```
...
```before
상권 매출 감소는 결국 지역 경제 침체로 이어진다. 시는 상반기에 임대료 지원을 시작했다.
```

```after
상권 매출이 줄면 일자리가 사라지고 지역 경제가 위축된다. 시는 상반기에 임대료 지원을 시작했다.
```
...
```before
이중구조를 더 벌려놓을 수 있다는 우려가 나오는 이유다. 노조는 다음 주 협상을 재개한다.
```

```after
그래서 이중구조를 더 벌려놓을 수 있다는 우려가 나온다. 노조는 다음 주 협상을 재개한다.
```
...
```before
신규 공정은 불량률을 2% 낮췄다. 그러나 과제도 남아 있다. 야간 작업 안전 기준이 아직 없다.
```

```after
신규 공정은 불량률을 2% 낮췄다. 다만 야간 작업 안전 기준이 아직 없다.
```
...
```before
경쟁은 심화되고 있다. 진입 장벽은 낮아지고 있다. 마진은 줄어들고 있다. 신규 업체는 매달 늘고 있다.
```

```after
경쟁은 심해졌다. 진입 장벽은 낮아지고 있다. 마진은 줄었다. 신규 업체는 매달 늘고 있다.
```
...
```before
정책이 실패했다는 점은 명확하다. 다음 예산은 두 배로 늘었다.
```

```after
정책은 실패했다. 다음 예산은 두 배로 늘었다.
```
...
```before
모델 크기는 더 이상 핵심이 아니다. 배포 속도도 더 이상 경쟁 요소가 아니다.
```

```after
모델 크기는 이제 핵심이 아니다. 배포 속도도 이제 경쟁 요소가 아니다.
```
...
```before
물론입니다! 다음은 배포 절차입니다: 먼저 서명 인증서를 갱신하고 만료일을 기록합니다. 그다음 패키지를 다시 빌드해 스토어에 제출합니다. 심사가 끝나면 릴리스 노트를 게시하고 지원팀에 알립니다. 도움이 되셨길 바랍니다.
```

```after
먼저 서명 인증서를 갱신하고 만료일을 기록합니다. 그다음 패키지를 다시 빌드해 스토어에 제출합니다. 심사가 끝나면 릴리스 노트를 게시하고 지원팀에 알립니다.
```
