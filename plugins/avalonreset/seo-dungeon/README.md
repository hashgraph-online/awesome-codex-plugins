...
[![CI](https://github.com/avalonreset/legends-seo-dungeon/actions/workflows/ci.yml/badge.svg)](https://github.com/avalonreset/legends-seo-dungeon/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![Version](https://img.shields.io/badge/version-2.2.5-blue)](CHANGELOG.md)
[![Runtime](https://img.shields.io/badge/runtime-Codex%20%7C%20Claude%20%7C%20Gemini%20%7C%20Grok-2ea44f)](dungeon/)

[Project Home](https://seodungeon.com) | [Source](https://github.com/avalonreset/legends-seo-dungeon) | [Releases](https://github.com/avalonreset/legends-seo-dungeon/releases)

Legends SEO Dungeon turns SEO audits into a 16-bit dungeon crawler. Enter a
domain, inspect the issues as demons, and use a local AI CLI to analyze or fix
them inside your local project.

This Legends build carries the public Claude SEO v2.2.5 engine refresh while keeping the
browser game Codex-default. The app can also spawn local Claude Code, Gemini CLI, or Grok
CLI when selected, and the underlying skills, agents, scripts, and references
remain plain files for compatible terminal agents.

The Dungeon version follows the bundled public Claude SEO release: **2.2.5**.
Dungeon-specific fixes within that engine version are identified by Git commits.

## Screenshots & Key Art

Game screenshots use fictional `example.com` data.

<table>
<tr>
<td width="50%"><a href="screenshots/title-screen.png"><img src="screenshots/title-screen.png" alt="Legends SEO Dungeon title screen with character and runtime selection"></a><br><em>Pick a CLI, pick a warrior, enter a domain</em></td>
<td width="50%"><a href="screenshots/gate-scene-full.png"><img src="screenshots/gate-scene-full.png" alt="Gate scene showing quest continuation options"></a><br><em>Continue a previous quest or begin a new one</em></td>
</tr>
<tr>
<td width="50%"><a href="screenshots/summoning-scene.png"><img src="screenshots/summoning-scene.png" alt="Legends SEO Dungeon audit loading hallway with the hero descending"></a><br><em>Run a fresh audit while the hero descends</em></td>
<td width="50%"><a href="screenshots/dungeon-hall.png"><img src="screenshots/dungeon-hall.png" alt="Dungeon hall showing SEO issue demons sorted by severity"></a><br><em>Browse SEO demons sorted by severity</em></td>
</tr>
<tr>
<td width="50%"><a href="screenshots/battle-scene.png"><img src="screenshots/battle-scene.png" alt="Turn-based battle scene with real-time Guild Ledger"></a><br><em>Battle demons with agent-powered fixes</em></td>
<td width="50%"><a href="assets/social-preview.png"><img src="assets/social-preview.png" alt="Legends SEO Dungeon social preview key art"></a><br><em>Share-ready Legends key art</em></td>
</tr>
</table>

## How It Works

1. Choose a local CLI runtime: Codex, Claude Code, Gemini CLI, or Grok CLI. Codex is
   selected by default.
2. Choose a character profile. Codex maps Warrior/Samurai/Knight to `xhigh`,
   `high`, and `medium`; Claude maps them to Opus/Sonnet/Haiku; Gemini maps
   them to Pro/Flash/Flash Lite. Grok uses your configured model with high/high/medium reasoning effort.
3. Enter a domain and local project path.
4. For Codex, arm YOLO Mode so it runs with `--dangerously-bypass-approvals-and-sandbox`.
...
Inside the browser game, the actual work happens through the selected local CLI
in your local environment. The dungeon bridge starts local CLI processes only;
it does not proxy model access and does not route through browser automation or
consumer-app wrappers. Outside the game, compatible terminal agents can use the
same `skills/`, `agents/`, and `scripts/` directly.

## SEO Engine

Source: [Daniel Agrici's public Claude SEO](https://github.com/AgriciDaniel/claude-seo),
MIT licensed. The engine pin and local adaptations are documented in
[the upstream audit](docs/UPSTREAM-SEO-AUDIT-2026-09-06.md).
See [third-party notices](THIRD-PARTY-NOTICES.md) and the
[original MIT license](LICENSE-CLAUDE-SEO).

From the repository, check readiness with `python scripts/runtime.py doctor --json`.
Prepare the isolated engine with `python scripts/runtime.py setup`.
Run a tool with `python scripts/runtime.py run <script.py> [arguments]`.
These are the cross-platform equivalents of upstream's `claude-seo` launcher.
The Codex installers bind installed prompts to the installed runtime's absolute path.


The bundled public Claude SEO v2.2.5 engine (plus published maintenance) has 25 sub-skills (21 core + 1 orchestrator + 1
framework integration + 2 extension mirrors), 18 portable sub-agents, 23 Codex
TOML profiles, and 53 upstream Python modules plus 2 Dungeon API adapters.

Full audits are treated as multi-agent work by default. Legends SEO Dungeon asks
the selected runtime to fan out specialist audit workers in parallel whenever
that runtime supports it, and delegated workers inherit the selected strength
profile: Warrior stays extra-high, Samurai stays high, and Knight stays medium.
...
- Python 3.10+
- Git
- At least one supported local CLI installed and signed in: Codex (default),
  Claude Code, Gemini CLI, or xAI Grok CLI. See [runtime setup](docs/RUNTIMES.md).

### Prepare the SEO Engine
...
This installs the engine in its own environment. Some integrations need their own
credentials; they are optional for starting the game.

### Install Codex Skills (Optional for the Game)
...
`--real-codex --position-codex-window`, Codex visible on the left, Legends SEO
Dungeon visible on the right, and no `--allow-foreground-mismatch`.
...
| `SEO_DUNGEON_GROK_CLI` | official installed Grok | Grok executable override; see [runtime setup](docs/RUNTIMES.md) |
...
Set a model variable to `default`, `auto`, or `none` to let that CLI use its
own configured default model.

### Project API Credentials

Legends SEO Dungeon treats the selected project folder as the credential source
for audit integrations. Add a `.env` or `.env.local` file at that project root
...
have one configured, an agent may use it quietly, but Legends SEO Dungeon does not
...
Use `AGENTS.md`, `skills/`, `agents/`, and `scripts/` as the source of truth.
The skill frontmatter and Python helpers are intentionally portable, and all
URL-fetching scripts keep the same SSRF protections across compatible runtimes.
...
legends-seo-dungeon/
  dungeon/                         # Phaser game and WebSocket bridge
    server/index.js                # Local CLI game bridge
...
SEO engine code is derived from Daniel Agrici's SEO skill suite and used under
the MIT license. Legends SEO Dungeon is an independent
Codex-default workflow with portable skill files.
