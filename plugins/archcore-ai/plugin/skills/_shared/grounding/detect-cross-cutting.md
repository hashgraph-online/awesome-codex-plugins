...
route to `/archcore:document`. In large mode, scan **repo-wide** — cross-cutting
patterns are inherently global, not limited to the selected domains.

**Deduplicate against authored conversion targets (Phase B).** Before a candidate reaches the preview, compare its constraint — the specific library, module, guard, or log symbol it mandates or forbids — against every authored conversion target of type `rule` that the assessment gate planned (`init/lib/sources.md`, `skills/_shared/tracks/import.md`). If an authored rule already governs the same constraint (same symbol or module named in the mandate/prohibition), **drop the synthesized candidate** and note it in the preview under the covering target: `"↳ authored rule <source> covers this — synthesis skipped."` Prefer the human-authored, human-confirmed rule over a heuristic paraphrase; never create both. If the authored source has the verdict `skip` or `reference`, do NOT dedup — no rule document comes from it, so the synthesized candidate stands.
...
On `y`, direct user to `/archcore:document` (passes a draft rule as context). Do NOT auto-invoke — the user should confirm the exact phrasing through the standard flow. On `n` or no trigger, skip silently.
