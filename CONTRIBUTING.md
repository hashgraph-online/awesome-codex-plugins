...
```bash
pipx install --force "plugin-scanner==3.0.56"
plugin-scanner scan . --format text
```

Expected reviewed wheel SHA256: `2ca83abebbecc556a624c7e7382c1afead6dbff2261cf41d8be7960d9b014c58`

If you want to verify the exact wheel before install:

```bash
rm -rf .hol-plugin-scanner-dist
python3 -m pip download --only-binary=:all: --no-deps --dest .hol-plugin-scanner-dist "plugin-scanner==3.0.56"
